            ...
            if (bus.hosts.devicestatus) {
                ...
                $deviceDiv.css({
                 ...
                 $(".IPCamera")*/.on("click", function () {
                    ...
                    var $ipvideotext = $("<div></div>").text('监控视频').css({
                        'width': '400px',
                        'float': 'left',
                        'margin-left': '200px'
                    });
                    var $ipvideoclose = $("<div></div>").text('关闭').css({
                        'float': 'right',
                        'width': '60px',
                        'margin-right': '40px',
                        'cursor': 'pointer'
...
var showBusInfo = function showBusInfo(follow) {
    ...
    var content = "<div style='width: 260px'><div style='float:left'><h5 style='margin:0 0 5px 0;padding:0.2em 0;font-weight:bold'>基本信息</h5>\
                <p style='font-size: 12px;'>车牌:" + $(this).attr("licenseplate") + "</p>\
                <p style='font-size: 12px;'>司机:老王</p>\
                <p style='font-size: 12px;'>车队:车队1</p>\
                </div>\
                <div style='float:left;padding-left: 12px'>\
                <h5 style='margin:0 0 5px 0;padding:0.2em 0;font-weight:bold'>运行信息</h5>\
                <p style='font-size: 12px;'>平均车速:30km/h</p>\
                <p style='font-size: 12px;'>最高车速:120km/h</p>\
                <p style='font-size: 12px;'>综合油耗:9.2L/100km</p></div></div>";
    d.content(content);
    d.show(follow);
}
    ...
    var content = "<div id='subgruopArtdialog'><h5 id='modifyTitle'></h5>\
		<form action='bus/changeSubGroup.do' method='post'>\
			...
			<ul class='forminfo'>\
				...
				<li><input type='button' name='saveap' class='btn' id='saveap' value='保存更改' /></li>\
				<!-- <br> -->\
			</ul>\
			</div>\
		</form></div>";
...
function networkoverview() {
    ...
    /*var marker = new BMap.Marker(new BMap.Point(121.233537, 31.102290), {icon: carIcon});  // 创建标注
     map.addOverlay(marker);*/

    intervalRefresh = setInterval(function mappoint() {
        var pointid = $("#pointId").val();
        if (typeof(pointid) != 'undefined' && pointid != '') {
            pointid = parseInt(pointid) + 1;
        } else {
            pointid = 1;
        }
        // $.post("mapPoint/getMapPointByID.do", {"mapPoint.id": pointid}, function (msg) {
        // $.post("mapPoint/getMapPointByHostsID.do", {}, function (msg) {
        $.post("mapPoint/getLastMapPointByHosts.do", {}, function (msg) {
            /*var mapId = msg.mapPoint.id;
             if (parseInt(mapId) > 99) {
             $("#pointId").attr('value', 1);
             } else {
             $("#pointId").attr('value', mapId);
             }
             var curr_point = new BMap.Point(msg.mapPoint.longitude, msg.mapPoint.latitude);
             marker.setPosition(curr_point);*/
            var data = msg.jsonResult.data;
            map.clearOverlays();
            for (var i = 0; i < data.length; i++) {
                var mapPoint = data[i];

                /*var licensePlate,driver,subGroupName="";
                 if(typeof(mapPoint.hosts.bus)=="object"){
                 bus = mapPoint.hosts.bus;
                ...
                // alert(mapPoint.apMAC);
                // var marker = new BMap.Marker(new BMap.Point(mapPoint.longitude, mapPoint.latitude), {icon: carIcon});  // 创建标注
                // map.addOverlay(marker);
                /*var sContent =
                 "<div style='width: 260px'><div style='float:left'><h5 style='margin:0 0 5px 0;padding:0.2em 0;font-weight:bold'>基本信息</h5>\
                 <p style='font-size: 12px;'>车牌:"+driver+"</p>\
                 <p style='font-size: 12px;'>司机:"+driver+"</p>\
                 <p style='font-size: 12px;'>车队:车队1</p>\
                 </div>\
                 <div style='float:left;padding-left: 12px'>\
                 <h5 style='margin:0 0 5px 0;padding:0.2em 0;font-weight:bold'>运行信息</h5>\
                ...
                translateCallback = function (point, mapPoint) {
                    ...
                    var sContent =
                        "<div style='width: 260px'><div style='float:left'><h5 style='margin:0 0 5px 0;padding:0.2em 0;font-weight:bold'>基本信息</h5>\
                        <p style='font-size: 12px;'>车牌:" + licensePlate + "</p>\
                        <p style='font-size: 12px;'>司机:" + driver + "</p>\
                        <p style='font-size: 12px;'>车队:" + subGroupName + "</p>\
                        </div>\
                        <div style='float:left;padding-left: 12px'>\
                        <h5 style='margin:0 0 5px 0;padding:0.2em 0;font-weight:bold'>运行信息</h5>\
                        <p style='font-size: 12px;'>平均车速:30km/h</p>\
                ...
                /*var curr_point = new BMap.Point(mapPoint.longitude, mapPoint.latitude);
                 BMap.Convertor.translate(curr_point,0,translateCallback);*/
                BMap.Convertor.translate(mapPoint, 0, translateCallback);
            }
    ...
    /*translateCallback = function (point){
     ...
     var sContent =
     "<div style='width: 260px'><div style='float:left'><h5 style='margin:0 0 5px 0;padding:0.2em 0;font-weight:bold'>基本信息</h5>\
     <p style='font-size: 12px;'>车牌:A151515</p>\
     <p style='font-size: 12px;'>司机:老王</p>\
     <p style='font-size: 12px;'>车队:车队1</p>\
     </div>\
     <div style='float:left;padding-left: 12px'>\
...
var editH = function editHosts() {
    ...
    var content = "<div id='apArtdialog'><h5 id='modifyTitle'></h5>\
		<form action='hosts/preConfig.do' method='post'>\
			...
			<ul class='forminfo'>\
				...
				<li><input type='button' name='saveap' class='btn' id='saveap' value='保存设置' /></li>\
				<!-- <br> -->\
				<li>\
					<input type='button' name='reboot' class='btn' id='reboot' value='重启' />\
					<input type='button' name='restore' class='btn' id='restore' value='恢复出厂设置' />\
				</li>\
			</ul>\
			</div>\
		</form></div>";
    ...
    $.post("hosts/getHostsByIdExclude.do", {jsonPara: JSON.stringify({id: $(this).attr("hostsid")})}, function (msg) {
        ...
        // msg = eval('(' + msg + ')');
        var data = msg.jsonResult.data;
        $("#modifyTitle").text(data[0].serialno + "无线参数设置");
...
function movie() {
    $.post("film/getAllFilms.do", {}, function (msg) {
        ...
        for (var i = 0, j = data.length; i < j; i++) {
            ...
            var td2 = $("<td></td>").text(u.publish ? "已发布" : "未发布").css("width", "15%");
            if (u.publish) {
                var td3 = $("<td></td>").html("<a href='#'>取消发布</a>").attr("publishStatus", u.publish).css("width", "10%");
            } else {
                var td3 = $("<td></td>").html("<a href='#'>发布</a>").attr("publishStatus", u.publish).css("width", "10%");
            }
            var td4 = $("<td></td>").html("<a href='#'>编辑</a>").addClass("edit").attr("movieId", u.id).css("width", "10%").on("click", editM);
					...
					<li><label>文件:</label>\
						...
						<div id='uploader1' class='wu-example'>\
							<div id='thelist1' class='uploader-list'></div>\
                                <div class='btns'>\
                                <div id='multi'>选择文件</div>\
                            </div>\
						</div>\
					</li>\
					<li><input type='button' name='saveap' class='btn' id='multiUpload' value='保存设置' /></li>\
				</ul>\
				</div>\
			</form></div>";
        d.content(content);
...
function seekHelpManager() {
    $.post("seekHelp_getAll.do", {}, function (msg) {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            tr.append($("<td></td>"));
            var gotoTdTag = $("<td></td>");
            var gotoTag = $("<input>").attr("type", "submit").val("进入地图");
